...
Base.log(), Base.Real(),Base.Imag() # All act as expected. Note that imag returns a scalar, not a bivector.
```
...
Even elements in the STA are isomorphic to 2X2 complex matrices (the full Pauli algebra), which is essentially how they are implemented here. (We acually unwrap the matrics and store the entries explicitly in a struct, which turns out to be slightly more efficient.)
